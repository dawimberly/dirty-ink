...
export function parseZip(query: string): string | null {
...
async function geocodeZippopotam(zip: string): Promise<GeoPoint | null> {
  const json = (await fetchJson(`https://api.zippopotam.us/us/${zip}`)) as {
...
/** Resolve a city, address, or US ZIP. Safe to call from the browser. */
